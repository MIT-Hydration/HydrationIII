...
#define VEL_LIM_RPM			150
#define MOVE_DISTANCE_CNTS	1000000	
#define NUM_MOVES			1
	...
	{ 
		...
		for (size_t i = 0; i < portCount; i++) {
			...
			for (size_t iNode = 0; iNode < myPort.NodeCount(); iNode++) {
				...
				theNode.Setup.ConfigLoad("/home/epi/Downloads/testhome.mtr");
				...
				while (!theNode.Motion.IsReady()) {
					if (myMgr.TimeStampMsec() > timeout) {
						printf("Error: Timed out waiting for Node %ld to enable\n", iNode);
						msgUser("Press any key to continue."); //pause so the user can see the error message; waits for user to press a key
						return -2;
					}
				}
				...
				if (theNode.Motion.Homing.HomingValid())
				{
					...
					{
						printf("Node [%ld] has not been homed.  Homing Node now...\n", iNode);
					}
					//Now we will home the Node
					theNode.Motion.Homing.Initiate();
					double positionmea = theNode.Motion.PosnMeasured;
                                        theNode.Motion.AddToPosition(-positionmea);
                                        theNode.Motion.PosnMeasured.Refresh();

                                        printf("FINALLY  Node %ld has already been homed, current position is: \t%8.0f \n", iNode, theNode.Motion.PosnMeasured.Value());
					...
					printf("Node[%ld] has not had homing setup through ClearView.  The node will not be homed.\n", iNode);
				}
			...
			for (size_t i = 0; i < NUM_MOVES; i++)
			{
				...
				//	theNode.Motion.MoveVelStart(1);  
					//Set Velocity Limit (RPM)
				        int x; 
                                        std::cout << "How much do you want to move it : "; 
                                        std::cin >> x ; 
                                        std::cout <<  x;
					printf("Moving Node \t%zi \n", iNode);
					theNode.Motion.MovePosnStart(x);
				//	theNode.Motion.PosnCommanded.Refresh();
				//	double myPos = theNode.Motion.PosnCommanded; 
					printf("Commanded Move is [%d] ERIC " , x);  
				...
				//	
				//	msgUser(" Press Enter to Stop .");					                                                       
                                                
					
					int y; 
					
					std::cout << "Stop and Enter New Velocity" ; 
					std::cin.ignore(); 
					std::cin >> y ;
				       	std::cout << y; 	
					printf("\n [%d] is the new velocity, now moving back to position ", y) ;
					...
					theNode.Motion.PosnMeasured.Refresh();
					double current  = theNode.Motion.PosnMeasured;
                        	        
					printf(" [%f] is the current Position relative to 0 ", current ); 
					double newMove = (x-current); 
					double newAcc = y * 2 ;
					theNode.Motion.MoveWentDone(); 
					theNode.Motion.AccLimit = newAcc ;          //Set Acceleration Limit (RPM/Sec)
                                        theNode.Motion.VelLimit = y;
					theNode.Motion.NodeStopClear();
					theNode.Motion.MovePosnStart(newMove);
